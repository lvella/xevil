...
GlowPalette::GlowPalette(ColorPalette &p_palette)
:PaletteModifier(p_palette)
{
  ...
  {
    m_palette.palentries[i].peRed=p_palette.palentries[i].peRed;
    m_palette.palentries[i].peGreen=p_palette.palentries[i].peGreen;
    m_palette.palentries[i].peBlue=p_palette.palentries[i].peBlue;
  }
...
void GlowPalette::clock()
{
  m_palette=  m_fogholdingpalette;
  m_amount+=m_dir;
  if (m_amount>GLOW_UPPERLIMIT)
  {
    m_amount=GLOW_UPPERLIMIT;
    m_dir=-1*GLOW_AMOUNT;
  }
  if (m_amount<GLOW_LOWERLIMIT)
  {
    m_amount=GLOW_LOWERLIMIT;
    m_dir=GLOW_AMOUNT;
  }
